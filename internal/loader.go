...
type TypeLoader struct {
	...
	ParseType       func(*ArgType, string, bool, bool) (int, string, string)
...
func (tl TypeLoader) ParseQuery(args *ArgType) error {
	...
	if args.QueryFields == "" {
		...
		for _, c := range colList {
			...
			f.Len, f.NilType, f.Type = tl.ParseType(args, c.DataType, c.IsEnum, args.QueryAllowNulls && !c.NotNull)
...
func (tl TypeLoader) LoadProcs(args *ArgType) (map[string]*Proc, error) {
	...
	for _, p := range procList {
		...
		_, procTpl.Return.NilType, procTpl.Return.Type = tl.ParseType(args, p.ReturnType, false, false)
...
func (tl TypeLoader) LoadProcParams(args *ArgType, procTpl *Proc) error {
	...
	for i, p := range paramList {
		...
		_, _, paramTpl.Type = tl.ParseType(args, strings.TrimSpace(p.ParamType), false, false)
...
func (tl TypeLoader) LoadColumns(args *ArgType, typeTpl *Type) error {
	...
	for _, c := range columnList {
		...
		f.Len, f.NilType, f.Type = tl.ParseType(args, c.DataType, c.IsEnum, !c.NotNull)
